...
def cloze_output_adaptor(y_true, y_pred):
    ...
    y_pred is (batch_size, max_n_masked, vocab_size)
    ...
    be padded to (batch_size by 5). A typical row of y_true may look like: [1, 3, -1, -1, -1] where -1 is the pad.
    ...
    For training on a single GPU or CPU, this is not necessary. The reason we would even bother to this reshaping
    ...
    in **its own (sub)batch**. The result is that y_true may get padded to 5, because the maximum no. of masked
    positions in the GLOBAL batch was 5, but one GPU may only pad to 3 because the maximum no. of masked positions it
    can see in its own (sub)batch is 3. And these two shapes are incompatible for loss and metric calculations.
    ...
    mask = tf.squeeze(tf.where(y_true == tf.cast(LABEL_PAD, y_true.dtype), False, True), axis=1)
